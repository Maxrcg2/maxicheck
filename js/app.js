...
const PUBLIC_APP_URL = "https://maxrcg2.github.io/maxicheck/";

// Referencias a los controles que la aplicación actualiza durante cada búsqueda.
const form = document.getElementById("movie-form");
const ageInput = document.getElementById("age");
const ageContextButton = document.getElementById("age-context-button");
const ageContextLabel = document.getElementById("age-context-label");
const ageContextValue = document.getElementById("age-context-value");
const ageContextMenu = document.getElementById("age-context-menu");
const ageContextClose = document.getElementById("age-context-close");
const useProfileAgeButton = document.getElementById("use-profile-age");
const profileAgeDescription = document.getElementById("profile-age-description");
const applyTemporaryAgeButton = document.getElementById("apply-temporary-age");
const openAgeSettingsButton = document.getElementById("open-age-settings");
...
const navSourcesButton = document.getElementById("nav-sources");
...
const accountCalculatedAge = document.getElementById("account-calculated-age");
...
let temporaryConsultationAge = null;
let sharedContentOpened = false;
...
// Calcula la edad en la fecha actual sin almacenar un número que quedaría
// desactualizado después del próximo cumpleaños.
...
function getProfileAge(profile = currentAccountProfile) {
    const calculatedAge = calculateAgeFromBirthDate(profile?.birthDate);
    if (calculatedAge !== null) return calculatedAge;
    const legacyAge = Number(profile?.defaultAge);
    return Number.isInteger(legacyAge) && legacyAge >= 1 && legacyAge <= 120 ? legacyAge : null;
}

function getEffectiveAge() {
    if (Number.isInteger(temporaryConsultationAge)) return temporaryConsultationAge;
    return getProfileAge();
}

function updateAgeContextInterface() {
    const profileAge = getProfileAge();
    const effectiveAge = getEffectiveAge();
    const isTemporary = Number.isInteger(temporaryConsultationAge);
    const recommendLabel = recommendAgeButton.querySelector("span:last-child");

    if (isTemporary) {
        ageContextLabel.textContent = "Consulta temporal";
        ageContextValue.textContent = `${effectiveAge} años`;
        ageContextButton.classList.add("age-context-button--temporary");
        recommendLabel.textContent = `Para ${effectiveAge} años`;
        recommendAgeButton.title = `Recomendar para ${effectiveAge} años`;
    } else if (profileAge !== null) {
        ageContextLabel.textContent = "Para mí";
        ageContextValue.textContent = `${profileAge} años`;
        ageContextButton.classList.remove("age-context-button--temporary");
        recommendLabel.textContent = "Recomendar para mí";
        recommendAgeButton.title = `Recomendar para mi edad: ${profileAge} años`;
    } else {
        ageContextLabel.textContent = "Sin edad configurada";
        ageContextValue.textContent = "Elegir edad";
        ageContextButton.classList.remove("age-context-button--temporary");
        recommendLabel.textContent = "Recomendar";
        recommendAgeButton.title = "Elige una edad para recibir recomendaciones";
    }

    useProfileAgeButton.disabled = profileAge === null;
    const useProfileAgeLabel = useProfileAgeButton.querySelector("strong");
    if (useProfileAgeLabel) {
        useProfileAgeLabel.textContent = profileAge === null
            ? "Usar mi edad"
            : `Usar mi edad (${profileAge} años)`;
    }
    profileAgeDescription.textContent = profileAge === null
        ? "Configura tu fecha de nacimiento en Cuenta."
        : `Edad calculada actualmente: ${profileAge} años.`;
}

function setAgeContextMenuOpen(isOpen, restoreFocus = false) {
    ageContextMenu.hidden = !isOpen;
    ageContextButton.setAttribute("aria-expanded", String(isOpen));
    document.body.classList.toggle("age-context-open", isOpen);
    if (isOpen) {
        const effectiveAge = getEffectiveAge();
        ageInput.value = Number.isInteger(temporaryConsultationAge) ? String(temporaryConsultationAge) : "";
        ageInput.placeholder = effectiveAge === null ? "Ej.: 12" : `Actual: ${effectiveAge}`;
        (useProfileAgeButton.disabled ? ageInput : useProfileAgeButton).focus({ preventScroll: true });
    } else if (restoreFocus) {
        ageContextButton.focus({ preventScroll: true });
    }
}

function updateCalculatedAgePreview() {
    const age = calculateAgeFromBirthDate(accountBirthDate.value);
    accountCalculatedAge.textContent = age === null
        ? "Configúrala para calcular tu edad automáticamente."
        : `Edad calculada actualmente: ${age} años. Se actualizará automáticamente en tu cumpleaños.`;
    accountCalculatedAge.classList.toggle("account-calculated-age--ready", age !== null);
}

async function refreshCurrentDetailForAge() {
    if (!currentDetailContent || resultToolbar.hidden) return;
    const content = currentDetailContent;
    const returnAction = detailBackAction;
    result.innerHTML = `<p class="loading-message">Actualizando la recomendación de edad…</p>`;
    try {
        await showContentDetails(content, getEffectiveAge(), { onBack: returnAction });
        scrollToResultStart();
    } catch (error) {
        result.innerHTML = `<p class="detail-error">No pudimos actualizar la recomendación.</p>`;
    }
}
...
function renderProfileEditor(user, profile) {
    ...
    accountBirthDate.value = profile.birthDate || "";
    updateCalculatedAgePreview();
...
// Limita el calendario a fechas reales compatibles con el rango de edad de la app.
(function initializeBirthDateLimits() {
    ...
    accountBirthDate.min = toLocalIso(oldestDate);
    accountBirthDate.max = toLocalIso(today);
}());

accountBirthDate.addEventListener("input", function() {
    accountBirthDate.setCustomValidity("");
    updateCalculatedAgePreview();
});

ageContextButton.addEventListener("click", function(event) {
    event.stopPropagation();
    setAgeContextMenuOpen(ageContextMenu.hidden);
});

ageContextClose.addEventListener("click", function() {
    setAgeContextMenuOpen(false, true);
});

useProfileAgeButton.addEventListener("click", function() {
    if (getProfileAge() === null) return;
    temporaryConsultationAge = null;
    updateAgeContextInterface();
    setAgeContextMenuOpen(false, true);
    refreshCurrentDetailForAge();
});

applyTemporaryAgeButton.addEventListener("click", function() {
    const age = Number(ageInput.value);
    if (!Number.isInteger(age) || age < 1 || age > 120) {
        ageInput.setCustomValidity("Escribe una edad entre 1 y 120 años.");
        ageInput.reportValidity();
        return;
    }
    ageInput.setCustomValidity("");
    temporaryConsultationAge = age;
    updateAgeContextInterface();
    setAgeContextMenuOpen(false, true);
    refreshCurrentDetailForAge();
});
...
openAgeSettingsButton.addEventListener("click", function() {
    setAgeContextMenuOpen(false);
    openAuthPanel("Configura tu fecha de nacimiento para calcular la edad automáticamente.");
    const preferencesSection = accountPreferencesForm.closest("details");
    if (preferencesSection) preferencesSection.open = true;
    window.setTimeout(function() { accountBirthDate.focus(); }, 0);
});

document.addEventListener("click", function(event) {
    if (!ageContextMenu.hidden && !event.target.closest(".form-field--age")) {
        setAgeContextMenuOpen(false);
    }
});

document.addEventListener("keydown", function(event) {
    if (event.key !== "Escape" || ageContextMenu.hidden) return;
    event.preventDefault();
    setAgeContextMenuOpen(false, true);
});
...
accountPreferencesForm.addEventListener("submit", async function(event) {
    ...
    const birthDate = accountBirthDate.value.trim();
    if (birthDate && calculateAgeFromBirthDate(birthDate) === null) {
        accountBirthDate.setCustomValidity("Selecciona una fecha de nacimiento válida.");
        accountBirthDate.reportValidity();
        return;
    }
    ...
    try {
        ...
        // Usa inmediatamente el perfil confirmado por el almacén para que la
        // fecha y la edad queden visibles sin esperar otro snapshot de Firestore.
        currentAccountProfile = profile;
        accountBirthDate.value = profile.birthDate || birthDate;
        updateCalculatedAgePreview();
        temporaryConsultationAge = null;
        updateAgeContextInterface();
        refreshCurrentDetailForAge();
...
authService.subscribe(function(state) {
    ...
    if (!state.user) {
        preferencesAppliedForUserId = null;
        temporaryConsultationAge = null;
        updateAgeContextInterface();
    }

    if (state.user && state.user.uid !== previousAuthenticatedUserId) {
        // Restaurar una sesión es un estado normal: la cuenta y el contador de
        // Mi lista ya lo comunican sin interrumpir con una notificación.
        if (!authPanel.hidden) {
...
profileStore.subscribe(function(state) {
    ...
    if (state.ready) openSharedContentFromUrl();
});

// Los enlaces compartidos identifican el título por tipo e ID. La ficha se
// reconstruye desde TMDB y utiliza la edad de la cuenta del destinatario.
async function openSharedContentFromUrl() {
    if (sharedContentOpened) return;
    const parameters = new URLSearchParams(window.location.search);
    const mediaType = parameters.get("media") === "tv" ? "tv" : parameters.get("media") === "movie" ? "movie" : "";
    const contentId = Number(parameters.get("id"));
    if (!mediaType || !Number.isInteger(contentId) || contentId <= 0) return;
    sharedContentOpened = true;
    ...
    result.innerHTML = `<p class="loading-message">Abriendo el título compartido…</p>`;

    try {
        const response = await fetch(`${API_BASE_URL}/${mediaType}/${contentId}/details`);
        if (!response.ok) throw new Error("No fue posible abrir el enlace compartido.");
        const content = normalizeContent(await response.json(), mediaType);
        movieInput.value = content.title;
        await showContentDetails(content, getEffectiveAge(), { onBack: resetApplication });
        scrollToResultStart();
    } catch (error) {
        resultToolbar.hidden = true;
        result.innerHTML = `<p class="detail-error">Este enlace no pudo abrir el título solicitado.</p>`;
    }
}
...
function buildContentShareUrl(mediaType, contentId) {
    const isLocalPreview = ["localhost", "127.0.0.1"].includes(window.location.hostname);
    const shareUrl = new URL(isLocalPreview ? PUBLIC_APP_URL : window.location.href);
    shareUrl.search = "";
    shareUrl.hash = "";
    shareUrl.searchParams.set("media", mediaType === "tv" ? "tv" : "movie");
    shareUrl.searchParams.set("id", String(contentId));
...
function renderMovieAutocomplete(results, query) {
    autocompleteResults = results.slice(0, 8).map(normalizeContent);
                ...
                return `
                    <button id="movie-autocomplete-option-${index}" class="movie-autocomplete__option" type="button"
                        ...
                        <span class="movie-autocomplete__copy">
                            ...
                            <small>${getContentTypeLabel(content)}${Number(content.vote_average) > 0 ? ` · ⭐ ${Number(content.vote_average).toFixed(1)}` : ""}</small>
                        </span>
                    </button>
                `;
            }).join("")}
...
movieAutocomplete.addEventListener("click", async function(event) {
    ...
    if (option.dataset.autocompleteAction === "select") {
        ...
        if (currentAccountProfile.openSuggestionDirectly) {
            searchResults.innerHTML = "";
            searchResults.style.display = "none";
            resultToolbar.hidden = false;
            backButton.hidden = false;
...
recommendationDetailsContent.addEventListener("click", function(event) {
    if (!event.target.closest(".recommendation-set-age")) return;
    closeInfoModal(true);
    setAgeContextMenuOpen(true);
});
...
function resetApplication() {
    ...
    try {
        ...
        cleanUrl.searchParams.delete("media");
        cleanUrl.searchParams.delete("id");
        window.history.replaceState(window.history.state, "", cleanUrl);
    } catch (error) {
        // Limpiar el enlace es una mejora de navegación, no un requisito funcional.
    }
...
navAdvancedButton.addEventListener("click", function() {
    setSourcesMenuOpen(false);
    openAdvancedSearch();
});

function setSourcesMenuOpen(isOpen, restoreFocus = false) {
    sourcesMenu.hidden = !isOpen;
    navSourcesButton.setAttribute("aria-expanded", String(isOpen));
    navSourcesButton.classList.toggle("catalog-nav__button--menu-open", isOpen);
    if (isOpen) {
        sourcesMenu.querySelector("a")?.focus({ preventScroll: true });
    } else if (restoreFocus) {
        navSourcesButton.focus({ preventScroll: true });
    }
}

navSourcesButton.addEventListener("click", function(event) {
    event.stopPropagation();
    setSourcesMenuOpen(sourcesMenu.hidden);
});

sourcesMenu.addEventListener("click", function(event) {
    if (event.target.closest("a")) setSourcesMenuOpen(false);
});

document.addEventListener("click", function(event) {
    if (!sourcesMenu.hidden && !event.target.closest(".catalog-nav")) setSourcesMenuOpen(false);
});

document.addEventListener("keydown", function(event) {
    if (event.key === "Escape" && !sourcesMenu.hidden) {
        event.preventDefault();
        setSourcesMenuOpen(false, true);
    }
});
...
async function showContentDetails(selectedMovie, age, options = {}) {
    ...
    currentDetailContent = selectedMovie;
    ...
    const contentShareUrl = buildContentShareUrl(mediaType, movieId);
    ...
    const hasConsultationAge = Number.isInteger(age) && age >= 1 && age <= 120;


    // La recomendación se calcula localmente comparando la edad con el mínimo.
    if (!hasConsultationAge) {

        recommendation =
            "ℹ️ Configura una edad para recibir una recomendación personalizada.";
        recommendationClass = "recommendation-summary--unknown";
        recommendationBadgeText = "Sin evaluar";
    ...
    recommendationDetailsContent.innerHTML = `
        ...
        ${hasConsultationAge
            ...
            : `<button type="button" class="recommendation-set-age">Elegir una edad de consulta</button>`
        }
    ...
    result.innerHTML = `
        ...
        <div class="movie-poster-zone">
            <div class="movie-poster-frame poster-interaction-cue">
                ...
                <button type="button" class="poster-share-button movie-action--share" data-share-url="${escapeHtml(contentShareUrl)}" aria-label="Compartir ${escapeHtml(title)}" title="Compartir este título">
                    <span aria-hidden="true">↗</span>
                </button>
        ...
        <section class="movie-actions" aria-labelledby="movie-actions-title">
            <div>
                <h3 id="movie-actions-title">Enlaces oficiales</h3>
                <p>Consulta la información oficial de este título.</p>
            </div>
    ...
    function bindMovieActions() {
        ...
        const shareButton = result.querySelector(".movie-action--share");
        const posterStatus = result.querySelector(".poster-actions-status");
        if (!saveButton || !copyTitleButton || !shareButton || !posterStatus) return;
        ...
        shareButton.addEventListener("click", async function() {
            const contentUrl = shareButton.dataset.shareUrl || buildContentShareUrl(mediaType, movieId);
            const shareData = {
                title: `${title} | MaxiCheck`,
                text: `Consulta ${title} (${year}) en MaxiCheck.`,
                url: contentUrl
            };
            ...
            try {
                if (navigator.share) {
                    ...
                    posterStatus.textContent = "Título compartido con su enlace directo.";
                } else {
                        ...
                        try {
                            await Promise.race([
                                ...
                                new Promise(function(_resolve, reject) {
                                    window.setTimeout(function() { reject(new Error("clipboard-timeout")); }, 1200);
                                })
                            ]);
                            copiedWithClipboardApi = true;
                        } catch (clipboardError) {
                            // Algunos navegadores bloquean Clipboard API en vistas locales.
                        }
                    }
                    ...
                    posterStatus.textContent = "Enlace directo copiado al portapapeles.";
                }
            } catch (error) {
                // Cancelar el menú de compartir no debe mostrarse como un error grave.
                posterStatus.textContent = "No se compartió el título.";
            }
        });
    }
